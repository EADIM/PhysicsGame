...
public class Fase03_ToggleInputField : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        shouldActivateInputContainer = gameState.GetComponent<Fase03_GameState>().States["Exploration"];
    }
    ...
    private void checkPointer(Vector3 pos)
    {
        ...
        if ( Physics.Raycast (ray,out hit,500.0f)) 
        {
            ...
            if ((tag == "Player" && !UIElement.isVisible && shouldActivateInputContainer && !Fase03_CameraController.isLMoving && !Fase03_CameraController.isRMoving))
            {
                toggleInputContainer();
            }
        }
    ...
    public void toggleInputContainer(){
        if (!UIElement.isVisible && shouldActivateInputContainer && canShow){
            UIElement.Show();
            Fase03_ChangePlayerPosition.canChangePosition = false;
            ...
            UIElement.Hide();
            Fase03_ChangePlayerPosition.canChangePosition = true;
            GameObject jc = Utils.GetChildWithName(references.Canvas, "Joysticks Container");
